---
title: Datos y análisis para resultados de `r params$estudiante`
...
params:
  estudiante: 
  # estudiante: Saderis-Carmona-Marte
  # estudiante: Claribel-Reyes-Ramirez
  # estudiante: Arisleydi-Mejia-De-La-Cruz
  # estudiante: "Ramona-Geraldo-Munoz"
  # estudiante: "Angel-Rolando-Gomez-Gonzalez"
  # estudiante: "Manuel-Enrique-Urena-Reyes"
  # estudiante: "Adrian-Rafael-Diplan-Montas"
  # estudiante: "Yenny-Mabel-Santana"
---
...
<!-- IMPORTANTE: no olvidar actualizar base de datos postgres -->

```{bash, eval=F, include=F, echo=F}
ssh ...
psql -U USUARIO -d aggregate
\copy "BIOGEOGRAFIAHORMIGAS202302_CORE" TO '/tmp/biog2002_core.csv' DELIMITER ',' CSV HEADER;
\copy "BIOG2002ID_CORE" TO '/tmp/biog2002id_core.csv' DELIMITER ',' CSV HEADER;
\q
scp ...:/tmp/biog2002_core.csv "/home/jose/Documentos/ODK_Briefcase_exports/biogeografia_hormigas_202302/Biogeografía Hormigas 202302_core_extraido_postgres.csv"
scp  ...:/tmp/biog2002id_core.csv "/home/jose/Documentos/ODK_Briefcase_exports/biogeografia_hormigas_202302_identificacion/Biogeografía Hormigas 202302 IDENTIFICACION_extraido_postgres.csv"
```
...
# Cargar formularios de ODK y listas de estudiantes
estudiantes <- read_csv(paste0(gh_content, repo_sem202302, "fuentes/practica-04/lista-de-estudiantes.csv"))
estudiantes$`Nombres y apellidos compatible ODK` <- sapply(1:nrow(estudiantes), function(x) {
  tolower(iconv(
    gsub(" ", "", estudiantes[x, "Nombres y apellidos"]),
    from = "UTF-8", to = "ASCII//TRANSLIT"))
})
estudiantes$`Nombres y apellidos compatible Params` <- sapply(1:nrow(estudiantes), function(x) {
  iconv(
    gsub(" ", "-", estudiantes[x, "Nombres y apellidos"]),
    from = "UTF-8", to = "ASCII//TRANSLIT")
})
odk_campo <- read_csv('odk/biogeografia_hormigas_202302/Biogeografía Hormigas 202302.csv')
odk_campo <- odk_campo %>% rename_with(~ gsub("atributoshexagono-|coordenada-", "", .))
odk_id <- read_csv('odk/biogeografia_hormigas_202302_identificacion/Biogeografía Hormigas 202302 IDENTIFICACION.csv')
odk_id <- odk_id %>% rename_with(~ gsub("datoscolecta-", "", .))
odk_campo_form_usuario <- read_csv('odk/biogeografia_hormigas_202302/Biogeografía Hormigas 202302_core_extraido_postgres.csv')
odk_campo_form_usuario$`_CREATOR_URI_USER` <- gsub(
  '(uid:)(.*)(\\|.*)', '\\2',
  odk_campo_form_usuario$`_CREATOR_URI_USER`)
odk_campo_form_usuario <- odk_campo_form_usuario %>%
  rename(KEY = `_URI`, usuario_ODK = `_CREATOR_URI_USER`,
         dia_de_remision = `_SUBMISSION_DATE`)
odk_id_form_usuario <- read_csv('odk/biogeografia_hormigas_202302_identificacion/Biogeografía Hormigas 202302 IDENTIFICACION_extraido_postgres.csv')
odk_id_form_usuario$`_CREATOR_URI_USER` <- gsub(
  '(uid:)(.*)(\\|.*)', '\\2',
  odk_id_form_usuario$`_CREATOR_URI_USER`)
odk_id_form_usuario <- odk_id_form_usuario %>%
  rename(KEY = `_URI`, usuario_ODK = `_CREATOR_URI_USER`,
         dia_de_remision = `_SUBMISSION_DATE`)
usuarios_odk <- read_csv('odk/nombres_para_odk.csv')
estudiantes_todos_datos <- estudiantes %>%
  inner_join(usuarios_odk %>% rename(`Nombres y apellidos` = full_name) %>%
               select(-clean_full_name))
# write_csv(
#   estudiantes_todos_datos,
#   paste0(fuentes_manuscrito, 'estudiantes_todos_los_datos.csv'))

# Formularios de campo rellenados por:
estudiantes_han_rellenado <- sort(estudiantes_todos_datos$`Nombres y apellidos`[
  match(unique(odk_campo$responsable),
        estudiantes_todos_datos$`Nombres y apellidos compatible ODK`)])
# tibble(`Nombres y apellidos` = estudiantes_han_rellenado) %>% estilo_kable(
#   titulo = 'Estudiantes que han rellenado formularios de campo',
#   nombres_filas = F)
```

## Formularios de `r estudiantes_todos_datos$'Nombres y apellidos'[estudiantes_todos_datos$'Nombres y apellidos compatible Params'==params$estudiante]`
...
# Supongamos que 'rutas_imagenes' es tu vector de rutas de imágenes
rutas_imagenes <- paste0('odk/biogeografia_hormigas_202302/', na.omit(mis_forms_campo$foto))

# Bucle para insertar cada imagen
tryCatch(
  expr = for(ruta in rutas_imagenes) {
    cat(paste0("![", gsub('odk.*media/', '', ruta), "](", ruta, ")\n\n"))},
  error = function(e) e)
```

## Generar matriz de comunidad y ambiental, prepararlas para cargarlas
