...
> En principio, las muestras de hormigas las identificaremos de manera conjunta en el aula y en tutorías.

# Asignación de manuscrito 1. Diseño de muestreo y colecta de datos de campo

En teoría, el primer elemento de **una buena investigación comienza con unas buenas preguntas** o, si prefieres, con unos buenos objetivos, o con ambos. Se supone que las preguntas son las que guían el resto del proceso de investigación desde el comienzo, por lo que formular preguntas y definir objetivos debería ser el primer paso (un "truquito": en ecología numérica, uno de los mejores lugares para formularse preguntas es, el campo).

El segundo elemento de una buena investigación, consiste en contar con un buen diseño de muestreo y una colecta de datos de campo realizada de forma sistemática. En mi opinión, considero esta parte la más importante de un trabajo de ecología numérica (incluso por encima de las preguntas), pues si colectamos buenos datos hoy, en el futuro podremos responder múltiples preguntas, incluso algunas que no hayamos formulado inicialmente.

En investigaciones que siguen diseños estructurados y protocolos de campo, **la sección "Metodología" (el "cómo" de un estudio) normalmente se compone de dos subsecciones**: una comúnmente denominada "Diseño de muestreo y colecta de datos de campo", y otra denominada "Técnicas de procesamiento y analíticas". Por tal razón, esta asignación tiene dos objetivos:

1. Redactar la primera subsección de la metodología de tu manuscrito, a la que podrías titular "Diseño de muestreo y colecta de datos de campo".

2. Colectar tus muestras.

Por lo tanto, serán dos los entregables de esta práctica:

- Tus muestras de hormigas, colectadas y registradas en plataforma en línea (ver detalles más abajo). Todavía **no** identificarás hormigas, sólo las colectaras y registrarás. Harás el trabajo de campo, y deberás demostrar que colectaste tus hormigas.
...
Colectarás 15 muestras de hormigas en igual número de hexágonos H3 de resolución 12, en el polígono núcleo del campus de la Universidad Autónoma de Santo Domingo (UASD) en su sede central; el diseño está basado en el trabajo @jose_ramon_martinez_batlle_2023_8404063, dentro del cual se encuentra el documento ["Clasificación de hexágonos del polígono núcleo del campus de la Universidad Autónoma de Santo Domingo (UASD) en su sede central, usando coberturas del suelo"](https://github.com/geofis/coberturas-suelo-uasd/blob/main/clasificacion-de-hexagonos.md). Dicho documento explica el procedimiento de generación de los hexágonos y su clasificación.
...
nombres_para_odk <- estudiantes %>%
  mutate(full_name = gsub('\\?', 'ñ', gsub("[[:space:]]{2,}", " ", id_estudiante)),
         clean_full_name = paste0('sem202401', clean_text(full_name))) %>%
